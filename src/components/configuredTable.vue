<template>
  <div>
    ...
    <div v-if="picked && !loading && !error">
      ...
      <table>
        ...
        <tr v-for="person in results" v-on:click="show = person">
          <a
            href="#"
            class="favorite"
            v-on:click="addFavorite(person)"
            v-bind:class="{ active: favorite.indexOf(person.id) != -1 }"
            >★</a
          >
      ...
      <div v-if="show">
        Выбран пользователь <b> {{ show.firstName }} {{ show.lastName }} </b>
        <br />
        Описание:<br />
        <textarea>
          {{ show.description }}
...
export default {
  ...
  methods: {
    sort(field) {
      ...
      if (this.sortKey === field) this.reverse = !this.reverse;
      else {
        this.sortKey = field;
        this.reverse = false;
      }
